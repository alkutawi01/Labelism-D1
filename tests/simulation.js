...
console.log('\nLangkah 5-6: cetak separa (Batch 1)');
...
check('Batch 1 dicipta', p1.status === 201 && p1.body.runNumber === 1, json(p1.body));
const run1 = (await api(`/api/print-runs/${p1.body.id}`)).body;
check('Batch 1 ada 8 label (M2 + L6, Tudung 0)', run1.units.length === 8, `dapat ${run1.units.length}`);
check('nama penerima Batch 1 mengikut turutan (Aisyah, Balqis)', run1.units.filter(u => u.variant_label === 'Saiz M').map(u => u.recipient_name).join() === 'Aisyah,Balqis', '');
...
check('Batch 2 dicipta', p2.status === 201 && p2.body.runNumber === 2, json(p2.body));
const run2 = (await api(`/api/print-runs/${p2.body.id}`)).body;
check('Batch 2 ada 6 label, bermula dari Dania', run2.units.length === 6 && run2.units.find(u => u.variant_label === 'Saiz M').recipient_name === 'Dania', '');
...
console.log('\nLangkah 10: packing Batch 1');
const s1 = await api(`/api/print-runs/${run1.id}/packing/start`, { method: 'POST', body: {} });
check('Batch 1 menjangka 8 label sahaja', s1.body.planned === 8, json(s1.body));
...
check('label Batch 2 ditolak dalam Batch 1 (sebut "Batch 2")', wrong.status >= 400 && /Batch 2/.test(wrong.body.error || ''), json(wrong.body));
let scanned = 0;
...
check('6 label Batch 2 discan', s2 === 6, `${s2}`);
const c2 = await api(`/api/print-runs/${run2.id}/packing/close`, { method: 'POST', body: {} });
check('Batch 2 lengkap, tiada yang kurang', c2.body.missing === 0, json(c2.body));
